// ai-settings.component.ts - FIXED with session integration
...
export class AISettings extends LitElement {
  ...
  /**
   * 🆕 Load configuration from user session
   */
  private loadConfig(): void {
    const session = userSessionManager.getCurrentSession();

    if (session && session.aiConfig) {
      console.log('✅ Loading AI config from session:', session.aiConfig);
      this.config = { ...DEFAULT_OLLAMA_CONFIG, ...session.aiConfig };

      // Sync with ollamaAIService
      ollamaAIService.updateConfig(this.config);

      logger.addLog('success', 'AI configuration loaded from session');
    } else {
      console.log('⚠️ No AI config in session, using defaults');
      this.config = { ...DEFAULT_OLLAMA_CONFIG };
    }
  }

  /**
   * 🆕 Save configuration to user session
   */
  private saveConfig(): void {
    try {
      const session = userSessionManager.getCurrentSession();
      if (!session) {
        console.warn('Cannot save AI config: No active session');
        return;
      }

      console.log('💾 Saving AI config to session:', this.config);
      ...
      console.log('✅ AI config saved to session');
    } catch (error) {
